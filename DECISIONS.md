...
| Redesign Jev questions per primitive guidance: options mapped to the product schema, no catch-all options, multi-label relations as Nouls, syntax-provable relations moved to tree-sitter, richer grounded state | AI / question design | 9/10 | First-pass questions in `experiments/jev-graphrag/jevgraph/questions.py` | Written without the primitive guidance (docs blocked); aimed at getting the pipeline running | no |
